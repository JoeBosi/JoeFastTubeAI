#!/usr/bin/env python3
"""Setup / preflight for JoeFastTubeAI.
...
- Silent on success: --check exits 0 with no output when everything's ready so
  that the skill doesn't spam "setup is complete" on every turn.
- Idempotent: re-running the installer is safe — it never clobbers existing
  keys and only appends missing ones.
- SETUP_COMPLETE=true in ~/.config/JoeFastTubeAI/.env tells us the user has been
...
# (or when you point the skill at a local file with no subtitles).
#
...
#
# Leave both blank to disable Whisper — JoeFastTubeAI will still work, but videos
...
def _check_file_permissions(path: Path) -> None:
    ...
    try:
        ...
        if mode & 0o044:
            sys.stderr.write(
                f"[JoeFastTubeAI] WARNING: {path} is readable by other users. "
...
def _read_key_from_file(path: Path, name: str) -> str | None:
    if not path.exists():
        return None
...
def _scaffold_env() -> bool:
    """Create ~/.config/JoeFastTubeAI/.env with placeholders if missing."""
...
def cmd_check() -> int:
    ...
    sys.stderr.write(
        f"[JoeFastTubeAI] setup incomplete ({'; '.join(parts)}). "
...
def cmd_install() -> int:
    ...
    if has_key:
        ...
        if installed_deps:
            print("[setup] installed dependencies; JoeFastTubeAI is fully set up.")
        return 0
    ...
    print("")
    print("  Without a key, JoeFastTubeAI still works but videos without captions come back frames-only.")
    return 3
